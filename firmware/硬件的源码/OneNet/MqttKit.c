...
uint8 MQTT_UnPacketRecv(uint8 *dataPtr)
{
	...
	if(type == MQTT_PKT_PUBLISH)
	{
		...
		if(strstr((int8 *)msgPtr + 2, CMD_TOPIC_PREFIX) != NULL)	//是否为命令下发路径
			status = MQTT_PKT_CMD;
		else
...
//
//	入口参数：	user：用户名/产品ID
//				password：密码：鉴权信息/apikey
//				devid：设备ID
//				cTime：连接保活时间
//				clean_session：离线消息清理标志
//				qos：重发标志
						...
						MQTT_PACKET_STRUCTURE *mqttPacket)
{
	...
	//在线后是否清理离线消息：1-清理	0-不清理--------------------------------------------
	if(clean_session)
	{
		flags |= MQTT_CONNECT_CLEAN_SESSION;
	}

	//异常掉线的情况下，服务器发送的topic------------------------------------------------------
	if(will_topic)
	{
	...
	//qos级别--主要用于PUBLISH（发布态）消息的，保证消息数据的传输-----------------------------
	...
	//主要用于PUBLISH(发布态)的消息，表示服务器要保留这次推送的消息，如果有新的订阅者出现，就把这消息推送给它。如果不设置，那么推送当前订阅者之前就释放了
	if(will_retain)
	{
...
uint8 MQTT_UnPacketCmd(uint8 *rev_data, int8 **cmdid, int8 **req, uint16 *req_len)
{
	...
	int8 *dataPtr = strchr((int8 *)rev_data + 6, '/');	//前6个字节为头部信息
	...
	*cmdid = (int8 *)MQTT_MallocBuffer(37);				//cmdid固定36字节，最后一位是字符串结束位
	if(*cmdid == NULL)
		return 2;
	...
	switch(*qos)
	{
		case MQTT_QOS_LEVEL0:									// qos0 have no packet identifier
			...
			*payload = MQTT_MallocBuffer(*payload_len + 1);
			...
			{
				MQTT_FreeBuffer(*topic);						//还需要把topic的内存释放掉
				return 255;
			}
...
//	返回参数：	0-成功		1-失败原因
//
//	说明：		接收到的Publish消息的QoS等级为1时需要Ack回复
//==========================================================
...
//	返回参数：	0-成功		1-失败原因
//
//	说明：		接收到的Publish消息的QoS等级为2时，先收到rec
//==========================================================
...
//	返回参数：	0-成功		1-失败原因
//
//	说明：		接收到的Publish消息的QoS等级为2时，先收到rec，再回复rel
//==========================================================
